...
GLuaLS combines a language server with a full-featured VSCode extension for Garry's Mod Lua development. The language server delivers core code intelligence to any LSP-compatible editor, while the VSCode extension adds GMod-specific tooling, an interactive settings panel, and an integrated debugger.
...
Most settings are configured through `.gluarc.json` in your workspace root. Use the interactive settings panel (**GLua: Open Settings**) or edit the JSON directly. See the [Configuration](/configuration/overview) section for the full reference.

Extension-level settings (annotation downloads, UI preferences) are under `gluals.*` in VS Code settings. See [Configuration overview](/configuration/overview#vscode-extension-settings) for the full list.
