...
local function UseSkin(name)
    return derma.GetSkinTable(name)
end
```
...
Higher priority wins. You normally do not need this unless you are annotating a function with overloads or a generic wrapper that can behave like several GMod APIs.
