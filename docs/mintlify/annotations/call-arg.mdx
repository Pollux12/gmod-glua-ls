---
title: "@call_arg"
description: Mark wrapper parameters so GLuaLS can understand names, callbacks, files, colors, panels, skins, and scripted-class metadata.
---

## Overview

`@call_arg` marks a function parameter that requires special handling. Use it when you write a wrapper and still want GLuaLS to understand the names, callbacks, file paths, colors, panels, skins, or scripted-class metadata passed through that wrapper.
...
| `gmod.load` | `include`, `addcsluafile`, `add_cs_lua_file`, `includecs`, `include_cs`, `require` | File load graph, dependency edges, and realm inference for load wrappers |
| `gmod.file_find` | `glob`, `search_path`, `path` | `file.Find` result tracking for dynamic loader loops |
...
Unknown domains and roles are stored as metadata but ignored until GLuaLS adds a consumer for them. This is intentional to prevent errors for outdated clients.
...
Calls through these wrappers feed the same dependency and realm inference as direct `include`, `AddCSLuaFile`, `IncludeCS`, and `require` calls. Literal paths add dependency edges; dynamic paths can still participate when GLuaLS can trace them from `file.Find` loops.
