...
Use backticks when a string argument should pick a class/type.
...
## Constructor-style generic capture

For APIs that accept either a class object or a class-name string:
...
In `` `T`|T ``, the `T` part handles class objects and the `` `T` `` part handles class-name strings.
