...
`@accessorfunc` marks a function as an accessor generator. When it is called, GLuaLS creates `GetPropertyName()` and `SetPropertyName()` methods on the target class. This matches GMod's built-in `AccessorFunc()`.
...
At present, getter/setter values are typed as `any` in the implementation; stronger inference from the backing field is planned.
