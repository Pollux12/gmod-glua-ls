...
---@outparam paramName Type
```

- **`paramName`** — a parameter on the same function (must match an `@param`).
- **`fieldPath`** — the field the function writes to. Use dots for nested fields. Leave it out to type the argument itself.
...
Leave off the field path when the function fills in the table you pass, instead of a field inside it:
...
---@param out table Table owned by the caller.
...
local h = layout.rowHeight  -- ✅ integer
```
