...
Annotations are special comments prefixed with `---@` that provide type and other key information to the language server. They appear above functions, variables, and class definitions to let GLuaLS understand your code's structure and provide accurate completions, type checking, and hover documentation
...
Annotations have no impact on runtime and are seen as regular comments by Garry's Mod — they are purely for the language server and documentation. They are based on the EmmyLua annotation style, with some extensions and changes for GMod-specific concepts. Full backwards compatibility with EmmyLua and LuaLS is maintained where possible.
