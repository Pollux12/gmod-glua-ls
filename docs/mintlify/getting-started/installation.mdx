...
When running `glua_ls` outside of the VSCode extension, you can specify the path to GMod annotations using the `--gmod-annotations-path` flag:
...
<Note>
  The `--gmod-annotations-path` flag accepts a path to the folder containing the GMod API annotations (typically downloaded by the VSCode extension). Use `none` to explicitly disable annotations loading.
</Note>
