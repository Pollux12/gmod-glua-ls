...
By default, multi-root workspace isolation is enabled. This means that each folder has its own `.gluarc.json`, and the language server analyzes each one separately. Settings, diagnostics, and file param defaults stay scoped to that folder.

Multi-root workspace isolation can be disabled (`workspace.enableIsolation`) to allow for easier cross-folder analysis, which may be useful for gamemode development. See [Configuration](/configuration/workspace) for details.
