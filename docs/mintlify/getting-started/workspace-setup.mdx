...
By default, GLuaLS disables multi-root workspace isolation. The language server merges workspace configs into one global baseline. The first workspace config wins scalar conflicts, arrays are unioned, and per-workspace file overrides still apply where needed.
