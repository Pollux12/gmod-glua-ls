---
title: Diagnostics
description: GLuaLS checks your code for errors, type mismatches, unused variables, realm mistakes, and API misuse.
---
...
`need-check-nil` and `unchecked-nil-access` understand validity guards such as `IsValid` and annotated guard wrappers. See [Guard metadata](/annotations/guard-metadata) for custom wrappers.

`missing-fields` and `missing-parameter` skip anything with a documented default. Write `---@field clipSize number = 30` or `---@param times number = 1` when a base class, a setup function, or the function itself fills the value in. See [@field](/annotations/field#default-values) and [@param](/annotations/param#default-values).
...
Network checks also work through aliases and wrappers around the standard GMod network functions. See [Network analysis](/language/network-analysis) for examples and settings.
