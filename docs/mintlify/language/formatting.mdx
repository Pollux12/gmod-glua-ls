---
title: Formatting
description: GLuaLS formats your GLua files with built-in presets or an external formatter.
---
import ConfigTip from "/snippets/config-tip.mdx"

## Overview

GLuaLS includes an opinionated Lua formatter. Format the current file with `Shift+Alt+F` (or right-click → **Format Document**).


## Built-in presets

Three built-in presets cover common style choices across the GMod community:

| Preset | Description |
|---|---|
| `default` | Standard, no specific style |
| `cfc` | [Follows CFC-Servers style guidelines](https://github.com/CFC-Servers/cfc_glua_style_guidelines) |
| `custom` | Fall through to `styleOverrides` entirely |

Set the preset in `.gluarc.json`:

<ConfigTip />
```json .gluarc.json
{
  "format": {
    "preset": "default"
  }
}
```


## Style overrides

Override individual rules within a preset using `styleOverrides`:

<ConfigTip />
```json .gluarc.json
{
  "format": {
    "preset": "default",
    "styleOverrides": {
      "indent_style": "tab",
      "indent_size": 4,
      "tab_width": 4,
      "max_line_length": 120,
      "trailing_table_separator": "smart",
      "call_arg_parentheses": "keep"
    }
  }
}
```

### Key style options

| Option | Values | Description |
|---|---|---|
| `indent_style` | `"tab"`, `"space"` | Indentation mode |
| `indent_size` | `1`–`8` | Number of spaces per indent level (when using spaces) |
| `tab_width` | `1`–`8` | Visual width of tabs |
| `quote_style` | `"none"`, `"single"`, `"double"` | Preferred quote style for string literals |
| `call_arg_parentheses` | `"keep"`, `"remove"`, `"remove_table_only"`, `"remove_string_only"`, `"always"` | How to handle parentheses around single-string/table call args |
| `max_line_length` | integer | Line wrap threshold |
| `trailing_table_separator` | `"keep"`, `"never"`, `"always"`, `"smart"` | Trailing separator in table constructors |

See [Configuration: Formatting](/configuration/formatting) for the full list.


## External formatter

Use an external formatting tool (e.g., `stylua` or `GLuaFixer`) instead of the built-in formatter:

<ConfigTip />
```json .gluarc.json
{
  "format": {
    "externalTool": {
      "program": "stylua",
      "args": ["-"],
      "timeout": 10
    }
  }
}
```

The external formatter receives file content over stdin and must write the formatted result to stdout.


## EditorConfig

GLuaLS can read `.editorconfig` files for indent settings. EditorConfig values override corresponding `styleOverrides` entries. Priority can be set within the configuration for `.editorconfig` vs `.gluarc.json` settings.


## Format on save

Enable format-on-save in VS Code:

```json VS Code settings
{
  "editor.formatOnSave": true,
  "[lua]": {
    "editor.defaultFormatter": "Pollux.gmod-glua-ls"
  }
}
```

## Disabling formatting

Disable the formatter entirely:

<ConfigTip />
```json .gluarc.json
{
  "format": {
    "enable": false
  }
}
```
