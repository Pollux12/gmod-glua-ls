...
description: Completion, inheritance, generated accessors, and navigation for Garry's Mod scripted classes.
---
...
| `lua/weapons/gmod_tool/stools/` | `TOOL` | Tool |
| `gamemode/` | `GM` | Gamemode |
| `gamemodes/*/schema/**/` | `SCHEMA` / `Schema` | Gamemode |
...
Schemas work automatically for any framework or gamemode. GLuaLS treats `SCHEMA` and `Schema` as aliases for one global schema table, merges members declared across schema files, and exposes gamemode hooks inherited from `GM`. The schema remains available to files outside the `schema/` directory, matching the usual runtime behavior of global schema systems.
...
See [Garry's Mod settings](/configuration/gmod#scripted-classes) for all supported fields.
...
| `fixedClassName` | A single class name shared by every matching authoring file |
| `isGlobalSingleton` | Makes the class global and its aliases resolve throughout the project, not only in matching authoring files |
| `aliases` | Additional global names that resolve to the same class |
| `superTypes` | Classes whose members and hooks this class inherits |
| `hookOwner` | Treats methods on the class global and its aliases as gamemode hook implementations |
