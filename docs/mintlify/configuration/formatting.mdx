...
> Note: This page shows common settings. For the complete list, inspect the JSON schema at `crates/glua_code_analysis/resources/schema.json` under `EmmyrcFormatStyleOverrides`.
...
There are no `gluals.format.*` VSCode settings — formatting is configured entirely through `.gluarc.json`. You can enable format-on-save with these built-in VS Code settings:
