...
| `gmod.autoLoadAnnotations` | `boolean \| null` | `null` | Override the extension's auto-load setting. Set `false` to disable even if the extension has it enabled |
...
| `gmod.network.diagnostics.typeMismatch` | `boolean` | `true` | Warn when read type doesn't match write type across realms |
| `gmod.network.diagnostics.orderMismatch` | `boolean` | `true` | Warn when net.Read order doesn't match net.Write order |
| `gmod.network.diagnostics.missingCounterpart` | `boolean` | `true` | Warn when a send block has no receive (or vice versa) |
