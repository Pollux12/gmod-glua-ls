...
| `gluals.strict.strictTypeCoercion` | `strict.strictTypeCoercion` | `boolean` | `false` | When enabled, requires exact types in param-type-mismatch — disables GLua-aware coercions (e.g. number/boolean → string, numeric string literal → number/integer) |
