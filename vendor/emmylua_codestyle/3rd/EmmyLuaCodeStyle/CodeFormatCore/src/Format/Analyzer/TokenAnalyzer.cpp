...
namespace {
bool IsSafeRedundantConditionParens(LuaSyntaxNode parExpr, const LuaSyntaxTree &t) {
    auto innerExpr = parExpr.GetChildSyntaxNode(LuaSyntaxMultiKind::Expression, t);
    if (!innerExpr.IsNode(t) || !innerExpr.IsSingleLineNode(t)) {
        return false;
    }

    auto parent = parExpr.GetParent(t);
    switch (parent.GetSyntaxKind(t)) {
        ...
        if (syntaxNode.IsNode(t)) {
            switch (syntaxNode.GetSyntaxKind(t)) {
                case LuaSyntaxNodeKind::ParExpression: {
                    if (f.GetStyle().remove_redundant_condition_parentheses
                        && IsSafeRedundantConditionParens(syntaxNode, t)) {
                        auto leftParentheses = syntaxNode.GetChildToken('(', t);
                        if (leftParentheses.IsToken(t)) {
                            Mark(leftParentheses, t, TokenStrategy::Remove);
                        }

                        auto rightParentheses = syntaxNode.GetChildToken(')', t);
                        if (rightParentheses.IsToken(t)) {
                            Mark(rightParentheses, t, TokenStrategy::Remove);
                        }
                    }
                    break;
                }
