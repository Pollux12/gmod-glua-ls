...
void LineBreakAnalyzer::AnalyzeExprList(FormatState &f, LuaSyntaxNode exprList, const LuaSyntaxTree &t) {
    ...
    auto parent = exprList.GetParent(t);
    if (f.GetStyle().break_multiline_call_expression_list
        && parent.GetSyntaxKind(t) == LuaSyntaxNodeKind::CallExpression
        && !exprList.IsSingleLineNode(t)) {
        for (auto expr: exprs) {
            BreakBefore(expr, t, LineSpace(LineSpaceType::Keep));
            AnalyzeExpr(f, expr, t);
        }

        auto rightParentheses = parent.GetChildToken(')', t);
        if (rightParentheses.IsToken(t)) {
            BreakBefore(rightParentheses, t, LineSpace(LineSpaceType::Keep));
        }
