...
- The main analysis pipeline order in `crates/glua_code_analysis/src/compilation/analyzer/mod.rs` is intentional:
  ...
  10. `CallSiteParamAnalysisPipeline`
  11. `DynamicFieldAnalysisPipeline` when dynamic-field inference is enabled
- Do not move realm/load/hook/network work between phases casually. That metadata must exist before flow and Lua analysis so caches are keyed with the right realm from the start. Some dynamic outparam fields must be seeded before unresolve, while full dynamic-field collection needs unresolve-refined aliases.
...
- Prefer annotation-level modeling over hardcoded behavior. Call roles use attributes such as `---@attribute call_arg`, `call_arg_field`, `overload_call_arg`, and `overload_call_arg_field`. Tests can load the fixture via `VirtualWorkspace::def_gmod_call_arg_builtins()`.
...
- Scripted classes and generated members: VGUI panels, `ENT`, `SWEP`, `TOOL`, `PLUGIN`, `AccessorFunc`, `NetworkVar`, Derma skins, class bases, and gamemode inheritance are modeled through indexed metadata and synthesized types/members. Prefer extending `gmod_scripted_class_test.rs` and related realistic tests over small ad-hoc cases.
...
- LSP handlers: definition, completion, hover, references, code lens, and custom string contexts should use analyzer metadata. If a handler needs a new fact, consider indexing it in `glua_code_analysis` first.
...
- Do not hardcode API behavior in multiple handlers when it belongs in annotations or indexed metadata.
...
- For tests that rely on annotation-driven call roles, load `def_gmod_call_arg_builtins()`.
